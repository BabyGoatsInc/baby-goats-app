-- ====================================================================
-- BABY GOATS COMPLETE DATABASE SCHEMA FOR SUPABASE
-- ====================================================================
-- Copy and paste this entire file into Supabase SQL Editor and run
-- This creates ALL tables needed for the complete Baby Goats social platform
-- ====================================================================
...
-- ====================================================================
-- CORE USER PROFILES TABLE (Enhanced Supabase Version)
-- ====================================================================
...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    location TEXT,
    bio TEXT,
    sport TEXT, -- Added for compatibility with existing code
    grad_year INT, -- Added for compatibility  
...
-- Friendships Table with Improved Constraints
...
-- Teams Table with Enhanced Validation
...
-- Team Members with More Robust Checks
...
-- Team Challenges with Enhanced Validation
...
CREATE INDEX IF NOT EXISTS idx_leaderboards_active_type ON leaderboards(is_active, type);

-- Team indexes
CREATE INDEX IF NOT EXISTS idx_teams_sport_region ON teams(sport, region);
...
-- Challenge indexes
CREATE INDEX IF NOT EXISTS idx_team_challenges_sport_active ON team_challenges(sport, is_active);
CREATE INDEX IF NOT EXISTS idx_team_challenges_active_dates ON team_challenges(is_active, start_date, end_date);
...
-- Function to Validate Team Challenge Participation
CREATE OR REPLACE FUNCTION validate_team_challenge_participation()
RETURNS TRIGGER AS $$
DECLARE 
    team_size INT;
...
BEGIN
    ...
    -- Validate team size
    IF team_size < challenge_record.min_team_size OR 
       team_size > challenge_record.max_team_size THEN
        RAISE EXCEPTION 'Team size does not meet challenge requirements';
    END IF;
...
-- Trigger to Enforce Team Challenge Participation Rules
CREATE TRIGGER enforce_team_challenge_participation
BEFORE INSERT ON team_challenge_participations
FOR EACH ROW
EXECUTE FUNCTION validate_team_challenge_participation();
...
CREATE POLICY "Team members can view team details" ON teams
...
INSERT INTO leaderboards (name, description, type, scope, time_period, sport_filter) VALUES
...
-- Initialize team statistics for existing teams
...
BEGIN
    RAISE NOTICE '====================================================================';
    RAISE NOTICE 'BABY GOATS DATABASE SETUP COMPLETE!';
    RAISE NOTICE '====================================================================';
    RAISE NOTICE 'Successfully created:';
    RAISE NOTICE '✅ 15 Tables with enhanced validation and security';
    RAISE NOTICE '✅ Row Level Security (RLS) policies';
    RAISE NOTICE '✅ Performance indexes for optimal speed';
    RAISE NOTICE '✅ Database functions and triggers';
    RAISE NOTICE '✅ Sample data for testing';
    RAISE NOTICE '';
    RAISE NOTICE 'Your Baby Goats social platform is now ready!';
    RAISE NOTICE 'All features are enabled: Live Chat, Leaderboards, Teams, Competitions';
    RAISE NOTICE '====================================================================';
END $$;