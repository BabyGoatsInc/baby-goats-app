-- Baby Goats Database Schema - ENHANCED PRODUCTION VERSION
-- Includes validation constraints, performance indexes, and security improvements
...
-- Add graduation year constraint (current year to 10 years in future)
ALTER TABLE profiles 
ADD CONSTRAINT valid_grad_year 
...
-- Add constraint to ensure title is not just whitespace
ALTER TABLE highlights 
ADD CONSTRAINT valid_title 
CHECK (LENGTH(TRIM(title)) > 0);

-- Add constraint to ensure video_url is a valid URL format
ALTER TABLE highlights 
...
-- Add constraint to ensure stat_name is not just whitespace
ALTER TABLE stats 
ADD CONSTRAINT valid_stat_name 
...
ADD CONSTRAINT valid_challenge_title 
CHECK (LENGTH(TRIM(title)) > 0);

ALTER TABLE challenges 
ADD CONSTRAINT valid_challenge_description 
...
-- Create unique index to prevent duplicate completions per day
...
-- PERFORMANCE INDEXES (Foreign Key Columns)
...
CREATE INDEX idx_likes_highlight_id ON likes(highlight_id);

-- ADDITIONAL PERFORMANCE INDEXES
CREATE INDEX idx_profiles_sport ON profiles(sport);
CREATE INDEX idx_profiles_grad_year ON profiles(grad_year);
CREATE INDEX idx_highlights_is_featured ON highlights(is_featured);
...
-- FUNCTION: Update highlight likes count
...
-- TRIGGER: Auto-update likes count on highlights
