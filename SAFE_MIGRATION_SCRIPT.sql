-- Baby Goats Database Schema - SAFE MIGRATION VERSION
-- Handles existing tables and adds only missing components
...
-- DROP TABLE IF EXISTS debug_ping CASCADE;
...
-- Add constraints if they don't exist (safe approach)
DO $$ 
BEGIN
...
-- RLS Policies for profiles (safe - will skip if exists)
...
-- Add constraints for highlights
DO $$ 
BEGIN
...
-- Add constraints for challenges
DO $$ 
BEGIN
...
-- 5. Create challenge completions table
...
-- Create unique index to prevent duplicate completions per day (safe)
...
-- TRIGGER FUNCTIONS (safe replacement)
...
-- Insert seed challenges (only if challenges table is empty)
...
-- Insert seed athletes (only if profiles table is empty of sample data)
...
-- Insert sample highlights (only if not already present)
...
-- Insert sample stats (only if not already present)
...
-- Insert sample challenge completions (only if not already present)
...
FROM challenges c 
WHERE c.category = 'relentless' 
AND NOT EXISTS (SELECT 1 FROM challenge_completions WHERE user_id = '00000000-0000-0000-0000-000000000001')
LIMIT 3;